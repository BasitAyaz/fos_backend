...
Route.get("/", async (req, res) => {
    try {
        ...
        let query = `SELECT * FROM ItemInOut ${whereClause} ORDER BY TDate DESC OFFSET ${offset} ROWS
  FETCH NEXT ${limit} ROWS ONLY;`

        const result = await pool.request().query(query);
        const count = await pool.request().query(`SELECT COUNT(*) AS total FROM ItemInOut ${whereClause}`)
        res.json({
